...
import { SimdikAgamaDetilComponent } from './simdik-agama-detil/simdik-agama-detil.component';

@Component({
  selector: 'app-simdik-agama',
  templateUrl: './simdik-agama.component.html',
  styleUrls: ['./simdik-agama.component.scss']
})
export class SimdikAgamaComponent {
  title = 'AGAMA';
  data: any = [];
  ...
  ngOnInit(): void {
    this.spinner.show();
    this.api.get('agama').subscribe(result => {
      this.data = result;
  ...
  getDetil(data: any) {
    ...
    let dialog = this.dialog.open(SimdikAgamaDetilComponent, {
      width: '500px',
      data: data,
